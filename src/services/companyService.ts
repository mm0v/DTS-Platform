    ...
    try {
      ...
      if (files && (files.companyRegistry || files.financialReports)) {
        ...
        try {
          ...
          const response = await API.post("/api/v1/company/add", {
            companyRequest: data,
          });
      ...
      try {
        ...
        if (API_ENDPOINTS.DEVELOPMENT_MODE) {
          console.log(
            "DEVELOPMENT MODE: Using fallback endpoint or mock response"
          );
          ...
          try {
            ...
            if (fallbackResponse.ok) {
              ...
              return {
                success: true,
                message:
                  "Development mode: Form submitted successfully (mock response)",
                data: fallbackData,
              };
            }
          } catch (fallbackError) {
        ...
        throw new Error(
          "All submission attempts failed. Please try again later or contact support."
        );
      }
    }
  },
