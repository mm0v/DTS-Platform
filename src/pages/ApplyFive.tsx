...
// Use your production key for both dev and prod since backend expects real tokens
const RECAPTCHA_SITE_KEY = "6LerN1MrAAAAAHK3l-g1D8z0377xlEUT9_SbfQv-";
...
export default function ApplyFive() {
  ...
  // Enhanced reCAPTCHA handlers with strict validation
  const handleRecaptchaChange = (token: string | null): void => {
    console.log('reCAPTCHA token received:', token ? `Token: ${token.substring(0, 20)}...` : 'No token');
    ...
    if (token && token.length > 0) {
      ...
      setRecaptchaError(getErrorMessage('recaptcha_required'));
      setErrors(prev => ({ ...prev, recaptcha: getErrorMessage('recaptcha_required') }));
    }
  };
  ...
  const handleRecaptchaExpired = (): void => {
    ...
    setRecaptchaToken("");
    setIsRecaptchaVerified(false);
    setRecaptchaError(getErrorMessage('recaptcha_expired'));
    setErrors(prev => ({ ...prev, recaptcha: getErrorMessage('recaptcha_expired') }));
  };
  ...
  const handleRecaptchaError = (): void => {
    ...
    setRecaptchaToken("");
    setIsRecaptchaVerified(false);
    setRecaptchaError(getErrorMessage('recaptcha_error'));
    setErrors(prev => ({ ...prev, recaptcha: getErrorMessage('recaptcha_error') }));
  };
  ...
  // Enhanced form validation with strict reCAPTCHA checks
  const validateForm = (): boolean => {
    const newErrors: Record<string, string> = {};
    ...
    // CRITICAL: Strict reCAPTCHA validation
    if (!recaptchaToken || recaptchaToken.trim().length === 0 || !isRecaptchaVerified) {
      newErrors.recaptcha = getErrorMessage('recaptcha_required');
    }

    // Declaration validation
    if (!formData.declaration?.dataIsReal) {
      newErrors.dataIsReal = getErrorMessage('required');
    }

    if (!formData.declaration?.permitContact) {
      newErrors.permitContact = getErrorMessage('required');
    }

    if (!formData.declaration?.privacyAcceptance) {
      newErrors.privacyAcceptance = getErrorMessage('required');
    }
  ...
  const handleConfirmModalYes = async (): Promise<void> => {
    ...
    // Double-check reCAPTCHA before submission
    if (!recaptchaToken || !isRecaptchaVerified) {
      setSubmissionError(getErrorMessage('recaptcha_required'));
      setShowConfirmModal(false);
      return;
    }
    ...
    try {
      ...
      // CRITICAL: Pass reCAPTCHA token to service with final validation
      console.log('Submitting with reCAPTCHA token:', recaptchaToken ? 'Present' : 'Missing');
      await companyService.submitCompanyData(dataToSubmit, files, recaptchaToken);

      // Clear form data after successful submission
      localStorage.removeItem("companyData");
      localStorage.removeItem("digitalAndFinancial");
      localStorage.removeItem("digitalReadiness");
      localStorage.removeItem("propertyLaw");
      localStorage.removeItem("restOfData");
  ...
  // Updated form validation - CRITICAL: Check all required fields including reCAPTCHA
  const isFormValid =
    formData.declaration?.dataIsReal &&
    formData.declaration?.privacyAcceptance &&
    formData.declaration?.permitContact &&
          ...
          <form className="space-y-8">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <label
                  htmlFor="propertyLawCertificate"
                  className={`w-full h-14 border rounded-lg flex items-center justify-between px-4 bg-gray-800/30 text-gray-400 text-sm cursor-pointer select-none ${errors.propertyLawCertificate ? "border-red-500" : "border-gray-600"
                    }`}
                >
                  <span className="truncate">
                    {formData.files?.propertyLawCertificate
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <label
                  htmlFor="financialStatement"
                  className={`w-full h-14 border rounded-lg flex items-center justify-between px-4 bg-gray-800/30 text-gray-400 text-sm cursor-pointer select-none ${errors.financialStatement ? "border-red-500" : "border-gray-600"
                    }`}
                >
                  <span className="truncate">
                    {formData.files?.financialStatement
              ...
              <div className="flex items-start">
                ...
                >
                  <input
                    ...
                    className="hidden"
                  />
                  <span className={`mt-1 w-5 h-5 flex items-center justify-center border rounded ${errors.dataIsReal ? "border-red-500" : "border-gray-400"
                    }`}>
                    {formData.declaration?.dataIsReal && (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="flex items-start">
                ...
                >
                  <input
                    ...
                    className="hidden"
                  />
                  <span className={`mt-1 w-5 h-5 flex items-center justify-center border rounded ${errors.permitContact ? "border-red-500" : "border-gray-400"
                    }`}>
                    {formData.declaration?.permitContact && (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="flex items-start">
                ...
                >
                  <input
                    ...
                    className="hidden"
                  />
                  <span className={`mt-1 w-5 h-5 flex items-center justify-center border rounded ${errors.privacyAcceptance ? "border-red-500" : "border-gray-400"
                    }`}>
                    {formData.declaration?.privacyAcceptance && (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
            ...
            {/* reCAPTCHA Component - CRITICAL SECTION */}
            <div className="space-y-4">
              ...
              <div className="text-center">
                {isRecaptchaVerified && recaptchaToken ? (
